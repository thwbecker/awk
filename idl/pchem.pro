...
WHILE(NOT  eof(1))  DO  BEGIN 
   ...
   readf, 1, s
   print,   dims, m, n, nrnd, timestep, time, b
   ...
   FOR i=0L, nrnd-1 DO BEGIN 
      ...
      readf, 3, j, x1, x2, tmp_tzz, tmp_txz, p, viscosity
      x(i) = x1 &  y(i) = x2
      u(i) = vv &  v(i)=vv2 & t(i)=temperature
      ;;comp(i) = composition
      pressure(i) = p
      visc(i) = viscosity
      ezz(i) = tmp_tzz/viscosity
      exz(i) = tmp_txz/viscosity
   ENDFOR 
   IF (NOT init)THEN BEGIN 
      xmax = max(x)
      ymax = max(y)
   ...
   ezz = rotate(reform(ezz, n+1, m+1), rotfac)
   exz = rotate(reform(exz, n+1, m+1), rotfac)
   
   
   IF(printing)THEN device, filename=dir+strcompress(string(printnr), /remove_all)+".eps", $
     /encapsulated, xsize=3*4.5*aspect, ysize=5, /TIMES, /BOLD, bits_per_pixel=16, /landscape, /color
   ...
   plot, x(*, 127),ezz(*,127)*visc(*, 127), title="s!ixx!n"

   ;;contour, t, x, y, title="temperature", /follow
   ...
   lowlim = 0.0 & highlim=0.9
   ;;shade_surf,exz(*,m*lowlim:highlim*m), $
     ;;x(*,m*lowlim:highlim*m), y(*,m*lowlim:highlim*m), $
     ;;ax=90,az= 0, shade=bytscl(exz(*,m*lowlim:highlim*m)), zstyle=4
   ;;contour, exz(*,m*lowlim:highlim*m), $
   ;;  x(*,m*lowlim:highlim*m), y(*,m*lowlim:highlim*m),/overplot, /follow
   ;;xyouts, 1.0, 1.0, "d!it!ne!ixz!n"
