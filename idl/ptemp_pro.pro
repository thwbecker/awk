...
PRO ptemp_pro, startmodel, stopmodel, printing, printsingle, important_temperature
   on_error,2                   ;Return to caller if an error occurs
   if n_elements(startmodel) le 0 THEN startmodel = 1023
   ...
   contourlevels = [important_temperature-0.09,  important_temperature-0.04, important_temperature,important_temperature+0.06,  important_temperature+0.11,important_temperature+0.16, important_temperature+0.21]
   ...
   flowlinetracing = 0
   velbc_arrows = 1
   fixed_vel_scaling = 1.0/6.0 &  nr_arrows=500
   ...
   FOR model=startmodel, stopmodel DO begin 
      ...
      WHILE(NOT  eof(1) AND NOT eof(2) AND (time LE stopattime))  DO  BEGIN 
         ...
         readf, 1, s
         IF(NOT printing) THEN print,  dims, n, m, nrnd, timestep, time, b
         ...
         IF(plotnow)THEN BEGIN 
            ...
            ENDIF ELSE  BEGIN 
               IF(stress)THEN BEGIN 
                  ...
                  tmp = fltarr(n)
                  FOR i=0, n-1 DO BEGIN 
                     IF(x(i, m-1) LT trenchat)THEN tmp(i) = 0.75e06/2800.0/9.81 ELSE $
                       tmp(i) = 0.75e06/3300.0/9.81
                  ENDFOR 
                  IF(printsingle)THEN BEGIN 
                     device, $
                     ...
                     plot,x(*,m-1),tzz(*,m-1)*tmp(*)/1.0e03, xtitle="x", ytitle="h [km]", $
                       charsize=0.75, yrange=[-1, 1]
                  ENDIF ELSE BEGIN 
                     plot,x(*,m-1),tzz(*,m-1)*tmp(*)/1.0e03, xtitle="x", ytitle="h [km]", $
                       title=dir, charsize=0.75, yrange=[-1, 1]
                  ENDELSE 
                  plots, trenchat, min(tzz(*,m-1)*tmp(*)/1.0e03) & plots, trenchat,max(tzz(*,m-1)*tmp(*)/1.0e03) ,$
                    /continue, linestyle=1
                  ;;plots, 0, 0 &  plots, max(x(*, m-1)), 0, /continue, linestyle=2
                  veldat(plotnr, 11) = total(abs(oldtopo - tzz(*,m-1)*tmp(*)/1.0e03))*xmax*670.0/double(n+1)
                  oldtopo = tzz(*,m-1)*tmp(*)/1.0e03
                  meantzz(plotnr) = mittelwert(tzz(trenchat/xmax*0.25*n:trenchat/xmax*0.75*n, m-1)*tmp(*)/1.0e03)
