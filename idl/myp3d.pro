;  $Id: plot_3dbox.pro,v 1.4 1997/01/15 03:11:50 ali Exp $
;
...
PRO myp3d, X, Y, Z, COLOR=COLOR, BACKGROUND=BACKGROUND,        $  
...
;  Check the values of the keywords  
   ...
   if(N_Elements(WALLCOLOR) eq 0)then  WALLCOLOR = COLOR  
   ...
   if(not KeyWord_Set(XRANGE))then     XRANGE=[Min(x, MAX=xmax), xmax]
   if(not KeyWord_Set(YRANGE))then     YRANGE=[Min(y, MAX=ymax), ymax]

;  Use SURFACE to set up the coordinates system, handle titles ect...     
    IF(gridstyle NE -1)THEN BEGIN 
    ...
    ENDIF ELSE BEGIN 
       ...
       k2 = fltarr(2, 2)
       k3 = fltarr(2, 2)
       Surface, k1, k2, k3 , /NODATA, /SAVE, TICKLEN=1,        $ 
         COLOR=COLOR, BACKGROUND=BACKGROUND, XTICK_GET=xt,  XRANGE=XRANGE, $
         YRANGE=YRANGE, $ 
         YTICK_GET=yt, ZTICK_GET=zt,ZRANGE=ZRANGE, SUBTITLE=SUBTITLE,     $
         TITLE=TITLE, YTITLE=YTITLE, ZTITLE=ZTITLE, XTITLE=XTITLE, _EXTRA=e 
    ENDELSE 
...
; And now plot the data along the walls of the box if requested
  ...
  if(KeyWord_Set(XY_PLANE))then $     
      Plots, X, Y, Replicate(!Z.Crange[0], Zcnt), /T3D, COLOR=WALLCOLOR, $
               LINESTYLE=XYSTYLE
  if(KeyWord_Set(YZ_PLANE))then $     
      Plots, Replicate(!X.Crange[1], Xcnt), Y, Z, /T3D, COLOR=WALLCOLOR, $
               LINESTYLE=YZSTYLE
  if(KeyWord_Set(XZ_PLANE))then $     
      Plots, X, Replicate(!Y.Crange[1], Ycnt), Z, /T3D, COLOR=WALLCOLOR, $
               LINESTYLE=XZSTYLE
; now plot the data     
     
   Plots, X, Y, Z, /T3D , PSYM=PSYM,LINESTYLE=LINESTYLE,$
     COLOR=40+bytscl(y)/4, _EXTRA=e
